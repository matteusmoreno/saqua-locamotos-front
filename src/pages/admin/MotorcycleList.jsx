...
import { 
  Bike, Plus, Search, Gauge, Hash, Fingerprint, 
  ShieldAlert, Loader2, MoreVertical, Edit, Camera, Power, PowerOff
} from 'lucide-react';
import toast from 'react-hot-toast';
import { MotorcycleService } from '../../services/motorcycleService';

// IMPORTAMOS O NOSSO HOOK DE CONFIRMAÇÃO
...
export function MotorcycleList() {
  ...
  const toggleMotorcycleStatus = async (motorcycle) => {
    ...
    // CHAMAMOS O NOSSO MODAL PREMIUM EM VEZ DO WINDOW.CONFIRM
    const isConfirmed = await confirm({
      title: isActivating ? 'Ativar Motocicleta' : 'Desativar Motocicleta',
      message: isActivating 
        ? `Tem a certeza que deseja colocar a moto ${motorcycle.plate} disponível para aluguer novamente?`
        : `Tem a certeza que deseja desativar a moto ${motorcycle.plate}? Ela não poderá ser vinculada a novos contratos.`,
      confirmText: isActivating ? 'Sim, Ativar' : 'Sim, Desativar',
      cancelText: 'Cancelar',
      isDanger: !isActivating // O botão fica vermelho se estiver a desativar
    });

    if (!isConfirmed) return; // Se o utilizador clicou em Cancelar, abortamos aqui.
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-white mb-2 flex items-center gap-3">
            <Bike className="text-brand-gold" size={32} />
            Frota de Motos
          </h1>
      ...
      {loading ? (
        ...
        <div className="bg-black-rich border border-gray-mid rounded-3xl p-12 text-center flex flex-col items-center">
          ...
          <p className="text-gray-400 mb-6 max-w-md">
            {searchTerm ? `Não encontramos resultados para "${searchTerm}".` : "Você ainda não possui motos cadastradas na frota."}
          </p>
          {!searchTerm && (
            <Link to="/admin/motos/nova" className="text-brand-gold font-bold hover:underline">
              Registar minha primeira moto
            </Link>
          )}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredMotorcycles.map((moto, index) => (
            <motion.div key={moto.motorcycleId || index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className={`bg-black-rich border hover:border-brand-gold/50 rounded-2xl p-6 transition-all hover:shadow-[0_10px_30px_rgba(250,204,21,0.05)] group relative ${moto.active ? 'border-gray-mid' : 'border-brand-red/30 opacity-75'}`}>
              ...
              <div className="flex items-start justify-between mb-6">
                <div className="flex gap-4 items-center">
                  <div className="w-16 h-16 rounded-xl bg-gray-darker border-2 border-gray-mid flex items-center justify-center overflow-hidden">
                    {moto.pictureUrl ? (
                      <img src={moto.pictureUrl} alt={moto.model} className="w-full h-full object-cover" />
                    ) : (
                      <Bike size={32} className="text-gray-500" />
                    )}
                  </div>
                  <div>
                ...
                <div className="flex flex-col items-end gap-2">
                  {moto.active ? (
                    <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-green-500/10 text-green-500 border border-green-500/20 uppercase">Ativa</span>
                  ) : (
                    <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-brand-red/10 text-brand-red border border-brand-red/20 uppercase">Inativa</span>
                  )}
                  {moto.available && moto.active ? (
                    <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-blue-500/10 text-blue-400 border border-blue-500/20 uppercase">Disponível</span>
                  ) : !moto.available && moto.active ? (
                    <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-orange-500/10 text-orange-400 border border-orange-500/20 uppercase">Alugada</span>
                  ) : null}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-4 mb-6">
                <div className="flex items-center gap-2 text-sm text-gray-400">
                  ...
                  <span className="text-white font-mono bg-gray-darker px-2 py-0.5 rounded">{moto.plate}</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-gray-400">
                  <Gauge size={16} className="text-gray-500" />
                  <span className="text-white">{moto.mileage} km</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-gray-400 col-span-2">
                  <Fingerprint size={16} className="text-gray-500" />
                  <span className="text-gray-300">Renavam: {moto.renavam}</span>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-gray-mid flex justify-between gap-3">
                <button 
                  ...
                  className={`flex items-center justify-center p-2 rounded-lg text-sm font-medium transition-colors ${moto.active ? 'bg-gray-dark hover:bg-brand-red/20 text-brand-red' : 'bg-green-500/10 hover:bg-green-500/20 text-green-500'}`}
                ...
                <button className="flex-1 flex items-center justify-center gap-2 bg-gray-dark hover:bg-brand-gold/10 text-brand-gold py-2 rounded-lg text-sm font-medium transition-colors border border-transparent hover:border-brand-gold/30">
                  <Camera size={16} /> Docs
                </button>
              </div>
            </motion.div>
