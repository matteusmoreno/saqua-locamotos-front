...
import { Bike, ArrowLeft, Loader2, Save, FileText } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black text-white flex items-center gap-3">Nova Moto</h1>
          <p className="text-gray-400 text-sm">Adicione um novo veículo à frota da Saqua Locamotos.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-black-rich border border-gray-mid rounded-3xl p-6 sm:p-8 shadow-xl space-y-8">
        <section>
          <div className="flex items-center gap-3 mb-6 pb-2 border-b border-gray-mid/50">
            <div className="p-2 bg-brand-gold/10 rounded-lg text-brand-gold"><Bike size={20} /></div>
            <h3 className="text-lg font-bold text-white">Detalhes do Veículo</h3>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InputField label="Marca *" error={errors.brand}>
              <input {...register('brand', { required: 'Marca é obrigatória' })} className={inputClassName} placeholder="Ex: Honda, Yamaha" />
            </InputField>
            
            <InputField label="Modelo *" error={errors.model}>
              <input {...register('model', { required: 'Modelo é obrigatório' })} className={inputClassName} placeholder="Ex: CG 160 Titan" />
            </InputField>
            
            <InputField label="Ano (Fabrico/Modelo) *" error={errors.year}>
              <input {...register('year', { required: 'Ano é obrigatório', pattern: { value: /^[0-9]{4}\/[0-9]{4}$/, message: 'Formato inválido (ex: 2021/2022)' } })} onChange={(e) => setValue('year', maskYear(e.target.value))} className={inputClassName} placeholder="Ex: 2021/2022" maxLength={9} />
            </InputField>
            
            <InputField label="Cor *" error={errors.color}>
              <input {...register('color', { required: 'Cor é obrigatória' })} className={inputClassName} placeholder="Ex: Vermelha" />
            </InputField>
          </div>
        </section>
        ...
        <section>
          <div className="flex items-center gap-3 mb-6 pb-2 border-b border-gray-mid/50">
            ...
            <h3 className="text-lg font-bold text-white">Identificação Legal</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <InputField label="Renavam *" error={errors.renavam}>
              <input {...register('renavam', { required: 'Renavam é obrigatório' })} className={inputClassName} placeholder="Número do Renavam" />
            </InputField>
            
            <InputField label="Chassi *" error={errors.chassis}>
              <input {...register('chassis', { required: 'Chassi é obrigatório' })} className={`${inputClassName} uppercase`} placeholder="Número do Chassi" />
            </InputField>
            
            <InputField label="Quilometragem (km) *" error={errors.mileage}>
              <input type="number" {...register('mileage', { required: 'Km obrigatória', min: { value: 0, message: 'Não pode ser negativo' } })} className={inputClassName} placeholder="Ex: 0" min="0" />
            </InputField>
            
            <div className="md:col-span-2">
              <InputField label="Disponibilidade Inicial">
                <select {...register('available')} className={`${inputClassName} appearance-none cursor-pointer`}>
